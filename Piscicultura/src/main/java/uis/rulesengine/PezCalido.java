...
@Rule//(name = "Pez calido", description = "Evalua la medición de la temperatura y si es menor a la permitida, devuelve una advertencia")
public class PezCalido implements PlatformRule {

	// valores fijos para esta regla
	private String AssociatedSensorType = "temperatura"; //Tipo de medicion asociado a esta regla
	...
	public String getDescription() {
		return "Evalua la medición de temperatura y si es menor a la permitida, devuelve una advertencia";
	}
	@Priority
						...
						for(Entry<String, String> x : ins.getDevices().entrySet()) // se busca en todos los devices asociados al estanque
							if(AssociatedActuatorType.equals(x.getValue().toString())){ // evalua si alguno de los devices del estanque existe un tipo que sea igual al tipo de esta regla
								...
								accion = "¡¡¡¡¡¡¡ "+ AssociatedActuatorType + ": la "+ AssociatedSensorType +" del "
										+ "estanque " + ins.getElement() + " esta muy fria, poblacion en riesgo !!!!!!!";
